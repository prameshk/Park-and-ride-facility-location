...
def runAssignment(pnr, type_eq):
    start1 =time.time()
    #pnrNodes = pnr #['4', '5', '6', '14', '15', '17', '19', '22']  # park-and-ride nodes
    readDemand()
    readNetwork()
    ...
    startAss = time.time()
    assignment("deterministic", "MSA", type_eq, accuracy=0.01, maxIter=10000)
    print("The assignment for ", len(nodeSet), " nodes and ", len(linkSet), " links took ", time.time() - startAss)
    return {l:linkSet[l].flow for l in linkSet}, calculateCost()
